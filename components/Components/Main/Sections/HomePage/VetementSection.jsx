import React from 'react'
import Image from 'next/image'

const VetementSection = () => {
  return (
    <div className='flex justify-between items-center  mx-auto pt-16 '>
    <div className='flex flex-row space-x-20'>
        <div className='flex flex-col pt-14 space-y-9'>
            <h1 className='text-4xl text-[#15213D] font-poppins font-semibold'>
                Vous souhaitez acheter des vêtements  <br />adaptées  <br /> à une certaine situation de handicap?
            </h1>
            <p className='text-[#263056]'>
                Vous êtes au bon endroit. <br /> Trouvez ici tout ce dont vous aurez besoin. 
            </p>
        </div>
        <div className='relative flex-1 items-center pt-10'>
            <Image src="/slides_photo/photo(7).svg" className='rounded-[24px]' width={400} height={500}/>
        </div>
    </div>
  </div>
  )
}

export default VetementSection